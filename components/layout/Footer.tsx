...
/** Brand social icons (inline SVG - no emojis). */
...
export default function Footer() {
  return (
    <footer className="relative border-t border-snow/10 bg-obsidian/60 backdrop-blur">
      ...
      <div className="mx-auto max-w-6xl px-6 py-14">
        <div className="grid gap-10 md:grid-cols-[1.5fr_1fr_1fr]">
          ...
          <Reveal>
            ...
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-silver">
              {SITE.tagline} Websites, SaaS, and AI built to put you above the rest.
            </p>
          </Reveal>

          {/* Nav */}
          <Reveal delay={0.1}>
            <h4 className="mb-4 text-xs font-bold uppercase tracking-widest text-snow">
              Navigate
            </h4>
            <ul className="space-y-3">
              {NAV_LINKS.map((l) => (
                <li key={l.href}>
                  <Link
                    href={l.href}
                    className="cursor-pointer text-sm text-silver transition-colors hover:text-crimson"
                  >
                    {l.label}
                  </Link>
                </li>
            ...
            <div className="flex gap-3">
              {SOCIALS.map((s) => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.label}
                  className="flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border border-snow/10 bg-charcoal/50 text-silver transition-all duration-200 hover:border-crimson/40 hover:text-crimson hover:shadow-glow-soft"
                >
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-snow/10 pt-6 text-xs text-silver-dim sm:flex-row">
          <p>
            © {new Date().getFullYear()} {SITE.name}. All rights reserved.
          </p>
          <p className="flex items-center gap-2">
            <span className="h-1.5 w-1.5 rounded-full bg-crimson shadow-glow" />
            {/* TODO: replace with your DKayLabs strapline. */}
            Engineered to win.
          </p>
        </div>
      </div>
